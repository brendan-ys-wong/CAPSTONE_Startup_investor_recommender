...
train_list = train_df.groupby('investor_name').count().sort_values("company_name", ascending=False).reset_index()['investor_name'].iloc[0:10]
len(train_df)
train_list

test_df = test.to_dataframe()
test_df = test_df[(test_df['funding_round_type'] == 'venture')]

len(test_df)
sum(test_df['company_name'])
test_df.groupby('investor_name').count().sort_values("company_name", ascending=False)
